...
export default function DriverTripPanel({ employeeId, employeeName }: DriverTripPanelProps) {
  ...
  const fetchChecklists = async (tripId: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const { data: created } = await db.from('trip_checklists').insert(allItems).select();
      if (created) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-black text-foreground">Minhas Viagens</h2>
      </div>

      <div className="bg-card rounded-2xl p-8 shadow-lg border border-border max-w-lg">
        <div className="flex items-center gap-3 mb-6">
          <span className={`w-4 h-4 rounded-full ${activeTrip ? 'bg-green-500 animate-pulse' : 'bg-muted'}`} />
          <span className="font-bold text-foreground text-lg">
            {activeTrip ? 'Viagem em andamento' : 'Nenhuma viagem ativa'}
          </span>
          {activeTrip && (
            <span className={`ml-auto text-xs font-bold px-2 py-1 rounded-full ${isDeliveryMode ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
              }`}>
        ...
        {activeTrip ? (
              ...
              <div className="bg-amber-50 rounded-xl p-4 space-y-2 border border-amber-200">
                ...
                {dailyChecklist.map(item => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-lg text-left text-sm transition-all ${item.checked ? 'bg-green-100 text-green-800' : 'bg-card text-foreground hover:bg-muted'
                      }`}
                  >
                    <span className={`w-5 h-5 rounded border-2 flex items-center justify-center text-xs ${item.checked ? 'bg-green-500 border-green-500 text-white' : 'border-muted-foreground/30'
                      }`}>
                      {item.checked && '✓'}
                    </span>
                    {item.label}
            ...
            {showSOS && (
              <div className="bg-red-50 rounded-xl p-4 space-y-3 border border-red-200">
                ...
                <select
                  ...
                  className="w-full p-2 rounded-lg border border-red-200 text-sm bg-card"
                >
                ...
                <textarea
                  ...
                  className="w-full p-2 rounded-lg border border-red-200 text-sm bg-card resize-none h-20"
                />
                <button
                  onClick={sendSOS}
          ...
          <div className="space-y-3">
            <div>
              <label className="text-sm font-bold text-foreground mb-1 block">🚗 Selecione o Veículo</label>
              <select
                value={selectedVehicleId}
                onChange={e => setSelectedVehicleId(e.target.value)}
                className="w-full p-3 rounded-xl border border-border text-sm font-bold bg-card"
              >
            ...
            <button
              ...
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all text-lg disabled:opacity-50"
            >
      ...
      <div className="bg-card rounded-2xl p-6 shadow-lg">
        <h3 className="font-bold text-foreground mb-4 flex items-center gap-2">
          <Route className="w-5 h-5 text-primary" /> Viagens Recentes
        </h3>
        <div className="space-y-2 max-h-64 overflow-auto">
          {recentTrips.map(trip => (
            <div key={trip.id} className="flex items-center justify-between p-3 bg-muted rounded-xl text-sm">
              <div className="flex items-center gap-3">
                <MapPin className="w-4 h-4 text-muted-foreground" />
                <span>{formatTime(trip.started_at)}</span>
              </div>
              <div className="flex items-center gap-4 text-muted-foreground">
                <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${trip.montagem_status === 'concluida' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                  }`}>
